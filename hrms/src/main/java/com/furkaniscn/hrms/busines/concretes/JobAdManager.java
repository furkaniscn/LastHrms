...
import com.furkaniscn.hrms.busines.abstracts.JobAdService;
import com.furkaniscn.hrms.core.utilities.results.*;
import com.furkaniscn.hrms.dataAccess.abstracts.*;
...
public class JobAdManager implements JobAdService {
	...
	public JobAdManager(JobAdDao jobAdDao, JobPositionDao jobPositionDao, EmployerDao employerDao, CityDao cityDao,
			WorkPlaceDao workPlaceDao, WorkTimeDao workTimeDao, JobAdActivationDao jobAdActivationDao,
			StaffDao staffDao) {
	...
	public DataResult<List<JobAd>> getActiveAndOrderLastDate() {
		return new SuccessDataResult<List<JobAd>>(this.jobAdDao.findByActiveOrderByLastDate(true),
				"Aktif iş ilanları tarihe göre listelendi");
	}

	@Override
	public DataResult<List<JobAd>> getActiveAndCompanyId(int id) {
		return new SuccessDataResult<List<JobAd>>(this.jobAdDao.findByActiveTrueAndEmployer_Id(id),
				"Şirkere göre aktif iş işanları listelendi");
	}

	@Override
	public DataResult<List<JobAd>> getByIsActiveAndPageNumberAndFilter(int pageNo, int pageSize,
			JobAdFilter jobAdFilter) {
		Pageable pageable = PageRequest.of(pageNo - 1, pageSize);
		return new SuccessDataResult<List<JobAd>>(this.jobAdDao.getByFilter(jobAdFilter, pageable).getContent(),
				this.jobAdDao.getByFilter(jobAdFilter, pageable).getTotalElements() + "");
	}
